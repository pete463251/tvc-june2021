...
maxDegreesX = 4.5  #how far in degrees are servos allowed to move
maxDegreesY = 4.5
...
###  calibrate loop #########
for n in range(64):  #should be range 85 or so
...
print("This is the H13 script, confirm H13 motor")
while( True ):
...
print("This is the H13 script, confirm H13 motor")
print("H13: Fire when ready - no more messages will be given.")
...
Kp = 0.5*3.14/180     #offset by  3.14/180 to convert to radians
Kd = 0.13            #no need to offset because return value for gx, gy, gz is in rad/s
...
while True:
    ...
    #print deltaT
    if( (time_now - time_start)> 15400 ):  # motor burns for 7.1s, don't need to update servos after that
    	motorburnoutcheck = False    
	if( (time_now - time_start)> 18900 ):  #for weight=21oz=590g, optimal delay is about 3.6s after burn out depending on nose cone shape, ~s burn time after detecting launch get here; include sleep delay
		...
		if( time_now - time_start > 162000 ):  #flight time expected to be 135s approximately
			break
	...
	#print "H13[ forceIndex ] = " +  str(H13[forceIndex]) + ", and forceFactor = " + str( forceFactor )
